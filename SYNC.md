...
The Root Cause: "Slow Motion" Timestamps

Pipeline forces the compositor output to framerate=30/1. Because of this strict cap, the compositor assigns timestamps for exactly 30fps (0ms, 33ms, 66ms, 100ms...). However, because 1080p software mixing is extremely heavy, WSL2 can likely only process about 15 frames in one real-time second.
Here is the fatal flaw: The compositor still strictly applies the 30fps timestamps to those 15 frames. It assigns them 0ms, 33ms... up to 500ms. So, in 1 second of real time, your pipeline produces only 0.5 seconds of video time. When these frames arrive at the WebRTC peer, the browser obeys the timestamps and plays them back. Because it's receiving 0.5 seconds of video every 1 second, it plays the video in slow motion, immediately generating that massive accumulating delay you are seeing on the counter!
To solve this, we must ensure the timestamps generated by the compositor perfectly match the amount of real-time passing.

* Lowered Framerate to 15fps: changed framerate=30/1 to framerate=15/1 across the pipeline (while keeping the 1920x1080 resolution). This halves the CPU load on WSL2. More importantly, when the compositor generates 15 frames per second, it will stamp them at 15fps (0ms, 66ms, 133ms...), so they span exactly 1.0 real-time seconds. The WebRTC receiver will now play them back in perfect real-time.
* Re-applied Decoupling Queues: re-added the queue max-size-buffers=1 leaky=downstream directly after the videotestsrc. This guarantees the background generator is completely immune to any remaining downstream hiccups.
* Re-applied Low-Latency Encoders: changed the huge 1.5-second queues back to strict 300ms time-limits to enforce absolute zero-latency drops if the x264enc falls behind.
