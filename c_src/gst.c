...
static void on_decoded_pad(GstElement *decodebin, GstPad *pad, gpointer user_data) {
    ...
    if (g_str_has_prefix(name, "video")) {
        ...
        peer->v_scale = scaler;
        gst_bin_add_many(GST_BIN(state.pipeline), converter, scaler, NULL);
        ...
        GstPadLinkReturn ret3 = gst_pad_link(scale_src, comp_pad);
        g_printerr("DEBUG: video gst_pad_link results: pad->conv_sink=%d, conv_src->scale_sink=%d, scale_src->comp_pad=%d\n", ret1, ret2, ret3);
